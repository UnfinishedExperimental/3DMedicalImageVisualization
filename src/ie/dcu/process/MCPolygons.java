...
public class MCPolygons {
    ...
    Map<Point3D, Integer> hashToIndex = new HashMap<>(1_000_000);
    List<Point3D> verticies = new ArrayList<>(1_000_000);
    ...
    public void initiateMCProcess(File[] fileSelections, String currentDir, String dataFolder, boolean rawImage) {
        ...
        for (int i = 0; i < totalSlices - 1; i++) {
            ...
            System.out.println("triangle count " + triTotalList.size());
            if (rawImage) {
        ...
        System.out.println("Total time to process the data set  : " + (float) ((end - start) / 1000f) + " seconds");
    }

    private void createCTMFile(String dataFolder) {
        try (OutputStream fos = new FileOutputStream(dataFolder + ".ctm")) {
            CtmFileWriter writer = new CtmFileWriter(fos, new MG2Encoder(), 8);
            Mesh newM = new Mesh(verticesCTM, null, indicesCTM, EMPTY_ATTRIBUTES, EMPTY_ATTRIBUTES);
    ...
    private void saveVerticesCTM() {
        verticesCTM = new float[verticies.size() * 3];
        System.out.println("verticesCTM" + verticesCTM.length / 3);

        int vertIndx = 0;
        for (Point3D v : verticies) {
            verticesCTM[vertIndx++] = v.x;
            verticesCTM[vertIndx++] = v.y;
            verticesCTM[vertIndx++] = v.z;
        }
    }
    ...
    private void generateMarchingCubePolygons(int i, int j, int index, Interpolated interpolated) {
        ...
        triTotalList.addAll(triangleSet);
 ...
 */
        ...
        for (int a = 0; MCLookUpTables.triTable[cubeindex][a] != -1; a += 3) {
            int v1 = addVertexAndGetIndex(vertlist[MCLookUpTables.triTable[cubeindex][a]]);
            int v2 = addVertexAndGetIndex(vertlist[MCLookUpTables.triTable[cubeindex][a+1]]);
            int v3 = addVertexAndGetIndex(vertlist[MCLookUpTables.triTable[cubeindex][a+2]]);
            Triangle3D trian = new Triangle3D(v1, v2, v3);
    ...
    private int addVertexAndGetIndex(Point3D vertex) {
        int hash = vertex.hashCode();
        Integer index = hashToIndex.get(vertex);
        if(index == null){
            index = verticies.size();
            hashToIndex.put(vertex, index);
...
//                    generateMarchingCubePolygons(gridCell);
    ...
    private void initResolution() {
        ...
        triTotalList = new ArrayList<>(2_000_000);
    }
}
